...
import click
import re
from aligner_v7 import sync_lip_validate, yaml, edict
...
def infer(
    ...
    # if os.path.exists(f'{os.path.dirname(os.path.dirname(pti_weight))}/f_space/stitch/decoder/snapshots'):
    #     pti_weight = f'{os.path.dirname(os.path.dirname(pti_weight))}/f_space/stitch/decoder/snapshots'

    if not pti_weight.endswith('pt') and not pti_weight.endswith("pth"):
        folder = pti_weight
        pti_weight = os.path.join(folder,
                                  sorted(os.listdir(pti_weight), key=lambda x: int(''.join(re.findall('[0-9]+', x))))[
                                      -1])
    ...
    pti_exp_name = \
    list(filter(lambda x: 'exp' in x or 'pivot' in x or 'pti' in x or 'bdinv' in x, pti_weight.split('/')))[0]
    pose_exp_name = pose_latent_path.split('/')[-2]

    if not save_path.endswith("mp4"):
        save_path = os.path.join(save_path,
                                 net_exp_name + '_' + pti_exp_name + '_' + pose_exp_name + '_' + landmarks_name + '_' + supress + '_v7.mp4')
