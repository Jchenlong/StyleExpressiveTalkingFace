...
def sync_lip_validate(
    ...
    print(f_space_path)
    print(f'{os.path.dirname(os.path.dirname(os.path.dirname(pti_weight_path)))}/f_space/f_space_ft.pt')
    if os.path.exists(f'{os.path.dirname(os.path.dirname(os.path.dirname(pti_weight_path)))}/f_space/f_space_ft.pt'):
        f_space_path = f'{os.path.dirname(os.path.dirname(os.path.dirname(pti_weight_path)))}/f_space/f_space_ft.pt'
        print(f_space_path)
    f_space = None
    if f_space_path:
    ...
    # landmark_offsets = landmark_offsets * 2
    # landmarks[:, 5:25, :] = id_landmarks[:, 5:25, :] + landmark_offsets
    # print(2.2)
    driving_files = None
    ...
    with imageio.get_writer(save_path, fps = 25) as writer:
        ...
        for i in p_bar:
            ...
            #if i not in exclude:
            #    continue
            landmark_offsets_list.append(landmark_offset.cpu())
            with torch.no_grad():
                offset = net(landmark_offset)
            #offset = smoothen_func(offset)
            offsets_list.append(offset.cpu())
    ...
    # landmark_offsets_list = np.concatenate(landmark_offsets_list,axis=0)
    # np.save('/data1/chenlong/yuyuhang/interface_out/1031/out/offsets.npy', offsets)
    # np.save('/data1/chenlong/yuyuhang/interface_out/1031/out/landmark_offsets.npy', landmark_offsets_list)
def evaluate(
             config_path: str,
             offset_weight_path: str,
             pti_weight_path: str
